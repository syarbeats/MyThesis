...
CurrentMap::CurrentMap(QWidget *parent, Qt::WFlags flags)
        ...
        waitingForFix(false)
{
    ...
    */ 
    ...
    resize(300, 300);
#endif
    setWindowTitle(tr("CurrentUserMap"));
    ShowCurrentMap();
...
CurrentMap::CurrentMap(qreal qLongitude, qreal latitude)
{
	//this->mLongitude = latitude
}

void CurrentMap::ShowCurrentMap()
{
	        int width = qMin(webView->width(), 640);
	        int height = qMin(webView->height(), 480);
	        QString url = GMAPS_STATICMAP_URL_TEMPLATE
	                              .arg(QString::number(this->mLatitude))
	                              .arg(QString::number(this->mLongitude))
	                              .arg(QString::number(width))
	                              .arg(QString::number(height));
	        webView->load(url);
}

void CurrentMap::delayedInit()
{
    if (usingLogFile) {
...
void CurrentMap::positionUpdated(const QGeoPositionInfo &info)
{
    QString heading = "?";
    QString speed = "?";
...
void CurrentMap::loadStarted()
{
    loading = true;
    webView->setUpdatesEnabled(false);
}

void CurrentMap::loadFinished(bool)
{
    loading = false;
